"""
高速版v2：カスタム成長率 × 時価総額分析
merge_asofの問題を回避し、groupbyベースで高速化
"""
...
# groupbyベースでマージ（高速）
print("  財務データをグループ化中...")
df_fin_sorted = df_fin.sort_values(['code', 'disclosed_date'])

# 各銘柄・開示日に対して、最新の財務データを取得
results_fin = []
for code in df['code'].unique():
    df_stock = df[df['code'] == code].copy()
    df_fin_stock = df_fin_sorted[df_fin_sorted['code'] == code].copy()

    if len(df_fin_stock) == 0:
        continue
    ...
    for idx, row in df_stock.iterrows():
        ...
        # この開示日以前の最新財務データを取得
        fin_match = df_fin_stock[df_fin_stock['disclosed_date'] <= disclosed_date]

        if len(fin_match) > 0:
            latest_fin = fin_match.iloc[-1]
            results_fin.append({
                'idx': idx,
                'equity': latest_fin['equity'],
                'bps': latest_fin['bps']
            })

df_fin_matched = pd.DataFrame(results_fin).set_index('idx')
df = df.join(df_fin_matched, how='inner')
...
# 各銘柄・開示日に対して、開示日以降の最初の価格を取得
results_price = []
for code in df['code'].unique():
    df_stock = df[df['code'] == code].copy()
    df_price_stock = df_price[df_price['code'] == code].copy()

    if len(df_price_stock) == 0:
        continue
    ...
    for idx, row in df_stock.iterrows():
        ...
        # 開示日以降の最初の価格
        price_match = df_price_stock[df_price_stock['date'] >= disclosed_date]

        if len(price_match) > 0:
            first_price = price_match.iloc[0]
            results_price.append({
                'idx': idx,
                'adjusted_close': first_price['adjusted_close']
            })

df_price_matched = pd.DataFrame(results_price).set_index('idx')
df = df.join(df_price_matched, how='inner')
