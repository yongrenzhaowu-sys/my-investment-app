...
print("="*80)
print("FF5ファクターローリング分析（2021-03～2026-03、5年間）")
print("="*80)
...
# BPS代替計算
print(f"  元のBPS有効数: {df_fins['BPS'].notna().sum()}")
...
print(f"  BPS代替計算後の有効数: {df_fins['BPS_Final'].notna().sum()}（+{df_fins['BPS_Final'].notna().sum() - df_fins['BPS'].notna().sum()}）")

# INV_Growth計算（直近2期間）
print("\n[5] INV_Growth計算（直近2期間）")
...
# 財務データと月次データをマージ（3ヶ月前まで遡る）
print("\n[6] データマージ（月末の3ヶ月前までに開示されたデータ）")
...
print(f"  ファクター計算完了")
print(f"  MarketCap有効: {df_merged['MarketCap'].notna().sum()}レコード")

# ローリングウィンドウ分析
print("\n[8] ローリングウィンドウ分析（12ヶ月ウィンドウ）")

# ウィンドウ定義
all_months = sorted(df_merged['YearMonth'].unique())
...
for w in windows:
    # ウィンドウ内のデータ
    window_data = df_merged[df_merged['YearMonth'].isin(w['months'])].copy()

    # 欠損値除外
    valid = window_data.dropna(subset=['MonthlyReturn', 'MarketCap', 'BM', 'ROE', 'Momentum_6M'])

    if len(valid) < 50:
        print(f"  ウィンドウ{w['window_id']} ({w['start']}～{w['end']}): スキップ（データ不足: {len(valid)}銘柄）")
        continue

    # MKT（市場リターン、等ウェイト）
    mkt = valid['MonthlyReturn'].mean() * 12  # 年率換算
...
# 保存
print("\n[9] 結果保存")
output_path = OUTPUT_DIR / "ff5_rolling_factors.csv"
