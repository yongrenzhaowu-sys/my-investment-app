"""
全銘柄データでFF5+モメンタムファクターを完全計算（改善版）

改善点:
- 財務データの利用可能性を緩和（月末の1ヶ月前までに開示されたデータを使用）
- INV_Growth計算を直近2期間の成長率で計算（より多くの銘柄をカバー）
- ルックアヘッドバイアスは防止しつつ、対象銘柄数を最大化
"""
import pandas as pd
import numpy as np
...
# 財務データと月次データをマージ（改善版：1ヶ月前まで遡る）
print("\n[6] データマージ（月末の1ヶ月前までに開示されたデータ）")
...
for idx, row in month_end.iterrows():
    ...
    # 1ヶ月前の日付（ルックアヘッドバイアス防止）
    one_month_before = month_end_date - timedelta(days=30)

    # この時点で利用可能な財務データ（開示日が1ヶ月前以前）
    available_fins = df_fins[(df_fins['Code'] == code) & (df_fins['DiscDate'] <= one_month_before)]
...
for ym in target_months:
    ...
    # CMA（Conservative Minus Aggressive）改善版
