"""
全銘柄データでFF5+モメンタムファクターを完全計算（完全版）

最終改善:
- BPS代替計算: BPS欠損時は Eq / 発行済株式数 で計算
- 期待される効果: 約3,200銘柄が復活 → 合計5,000銘柄以上をカバー
"""
import pandas as pd
import numpy as np
...
print("="*80)
print("FF5+モメンタムファクター完全計算（完全版、BPS代替計算）")
print("="*80)
...
for idx, row in month_end.iterrows():
    ...
    if len(available_fins) > 0:
        ...
        merged_data.append({
            ...
            'Eq': latest['Eq'],
            'TA': latest['TA'],
            'BPS': latest['BPS_Final'],  # BPS最終値を使用
...
print("\n[9] 結果サマリー")
print("\n最新ファクターリターン（月次）:")
print(df_factors[['MonthEnd', 'MKT', 'SMB', 'HML', 'RMW', 'CMA', 'WML', 'n_stocks', 'n_inv_valid']].to_string(index=False))
