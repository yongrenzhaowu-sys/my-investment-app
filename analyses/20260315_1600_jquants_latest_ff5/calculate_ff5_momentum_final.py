"""
全銘柄データでFF5+モメンタムファクターを完全計算（最終版）

最終改善:
- 財務データの利用可能性を大幅に緩和（月末の3ヶ月前まで遡る）
- より多くの銘柄をカバー（目標: 3,000銘柄以上）
- ルックアヘッドバイアスは防止（3ヶ月前までのデータなので問題なし）
"""
import pandas as pd
import numpy as np
...
print(f"  INV_Growth有効銘柄数: {df_fins[df_fins['INV_Growth'].notna()]['Code'].nunique()}")

# 財務データと月次データをマージ（最終版：3ヶ月前まで遡る）
...
for ym in target_months:
    ...
    # CMA（Conservative Minus Aggressive）最終版
