# -*- coding: utf-8 -*-
"""
October Rebalance Strategy - 2017-2025 (9 years) - FULLY CORRECTED VERSION

CRITICAL FIXES:
1. Compounding effect: Properly accumulates capital year-over-year
2. Adjusted price calculation: Manually calculates AdjC_Correct = C × AdjFactor
   (The adjusted_close column in curated data is NOT correctly adjusted!)
3. ROE quartile selection: Changed from Q4 (lowest ROE) to Q1 (highest ROE)
   (Legacy version had the same bug - was selecting low ROE instead of high ROE!)
"""
import pandas as pd
import numpy as np
...
print('='*80)
print('October Rebalance Strategy - 2017-2025 (9 years) - FULLY CORRECTED')
print('='*80)
print('CRITICAL FIXES:')
print('  1. Adjusted prices: AdjC_Correct = C × AdjFactor')
print('  2. ROE quartile: Q1 (highest ROE) instead of Q4 (lowest ROE)')
print('  3. Compounding: Proper year-over-year capital accumulation')
...
df_prices = df_prices.rename(columns={
    ...
    'open': 'O',
    'close': 'C',
    'adjusted_close': 'AdjC_Wrong',  # This column is NOT correctly adjusted!
    'adjustment_factor': 'AdjFactor'
})

# CRITICAL: Calculate correct adjusted price manually
# The adjusted_close column in curated data does NOT follow close * adjustment_factor formula
...
print('\n[3] Running Backtest (with compounding)')
...
for i in range(len(rebalance_dates) - 1):
    ...
    # CRITICAL FIX: Q1 = highest ROE, not Q4!
    # With ascending=False ranking, Q1 contains highest values, Q4 contains lowest values
    candidates = valid_data[
        (valid_data['PBR_Quartile'] == 1) &
        (valid_data['ROE_Quartile'] == 1)  # Changed from 4 to 1!
    ].copy()
    ...
    capital_per_stock = current_capital / TARGET_POSITIONS  # Use current capital!
    ...
    new_capital = total_investment + total_pnl - tax
...
# Save results
annual_returns_path = OUTPUT_DIR / 'annual_returns_2017_2025_final.csv'
...
# Summary
summary = {
    'Strategy': 'October_Rebalance_2017_2025_Final',
...
summary_path = OUTPUT_DIR / 'performance_summary_2017_2025_final.csv'
...
for _, row in annual_returns_df.iterrows():
    print(f'  {row["Year"]}: Start {row["StartCapital"]:>12,.0f} -> End {row["EndCapital"]:>12,.0f} | Return {row["NetReturn"]*100:>6.2f}% | Tax {row["Tax"]:>10,.0f}')
