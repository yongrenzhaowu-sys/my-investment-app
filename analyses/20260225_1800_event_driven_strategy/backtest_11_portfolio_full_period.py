"""
ポートフォリオ最適化（全期間評価版）: ベース戦略 + レバレッジ戦略

目的: 全期間（2017-06～2026-01）で評価し、正確なMDDを取得

データ欠損の扱い:
- 2017-06～2017-10: レバレッジ戦略のみ（ベース戦略データなし）
- 2017-11～2025-10: 両戦略を配分通りに組み合わせ
- 2025-11～2026-01: ベース戦略のみ（レバレッジ戦略データなし）
"""
...
# 月初に正規化（重要: 同じ月に複数のデータポイントを防ぐ）
df_leverage['year_month'] = df_leverage['year_month'].dt.to_period('M').dt.to_timestamp()

print(f"ベース戦略: {len(df_base)}ヶ月（{df_base['year_month'].iloc[0].date()} ~ {df_base['year_month'].iloc[-1].date()}）")
print(f"レバレッジ戦略: {len(df_leverage)}ヶ月（{df_leverage['year_month'].iloc[0].date()} ~ {df_leverage['year_month'].iloc[-1].date()}）")
...
# 全期間を特定
all_months = sorted(set(df_base['year_month'].tolist() + df_leverage['year_month'].tolist()))
...
for month in all_months:
    ...
    # レバレッジ戦略のリターン
    leverage_return = df_leverage[df_leverage['year_month'] == month]['return'].values
    if len(leverage_return) > 0:
        leverage_ret = leverage_return[0]
        has_leverage = True
    else:
        leverage_ret = 0.0
        has_leverage = False
...
# データ状況の確認
print(f"\n期間別データ状況:")
both = df_full['has_base'] & df_full['has_leverage']
base_only = df_full['has_base'] & ~df_full['has_leverage']
leverage_only = ~df_full['has_base'] & df_full['has_leverage']

print(f"  両戦略あり: {both.sum()}ヶ月")
print(f"  ベースのみ: {base_only.sum()}ヶ月")
print(f"  レバレッジのみ: {leverage_only.sum()}ヶ月")
...
# レバレッジ戦略（データがない期間は0%リターン）
leverage_returns_full = df_full['return_leverage'].values
leverage_cum = (1 + leverage_returns_full).cumprod()
leverage_peak = pd.Series(leverage_cum).expanding().max()
leverage_dd = (leverage_cum - leverage_peak) / leverage_peak
leverage_mdd_full = leverage_dd.min()

leverage_total_return = leverage_cum[-1] - 1
leverage_years = len(leverage_returns_full) / 12
leverage_cagr_full = (1 + leverage_total_return) ** (1 / leverage_years) - 1
leverage_vol_full = pd.Series(leverage_returns_full).std() * np.sqrt(12)
leverage_sharpe_full = leverage_cagr_full / leverage_vol_full if leverage_vol_full > 0 else 0

print(f"\nレバレッジ戦略（全期間）:")
print(f"  年率リターン: {leverage_cagr_full*100:+.2f}%")
print(f"  最大DD: {leverage_mdd_full*100:.2f}%")
print(f"  シャープレシオ: {leverage_sharpe_full:.3f}")
...
for weight_base, weight_leverage in allocations:
    ...
    for i, row in df_full.iterrows():
        if row['has_base'] and row['has_leverage']:
            # 両戦略あり: 配分通り
            ret = row['return_base'] * weight_base + row['return_leverage'] * weight_leverage
        elif row['has_base']:
            # ベースのみ: ベース100%
            ret = row['return_base']
        elif row['has_leverage']:
            # レバレッジのみ: レバレッジ100%
            ret = row['return_leverage']
        else:
            # データなし: 0%
            ret = 0.0
...
output_dir = PROJECT_ROOT / 'analyses/20260225_1800_event_driven_strategy/results_portfolio_full_period'
...
with open(output_dir / 'optimal_portfolio.txt', 'w', encoding='utf-8') as f:
    f.write(f"ポートフォリオ最適化結果（全期間評価: {all_months[0].date()} ~ {all_months[-1].date()}）\n")
    f.write("="*80 + "\n\n")
    f.write(f"評価期間: {len(all_months)}ヶ月\n")
    f.write(f"  両戦略あり: {both.sum()}ヶ月\n")
    f.write(f"  ベースのみ: {base_only.sum()}ヶ月\n")
    f.write(f"  レバレッジのみ: {leverage_only.sum()}ヶ月\n\n")
    ...
    f.write(f"レバレッジ戦略（単体、全期間）:\n")
    f.write(f"  年率リターン: {leverage_cagr_full*100:+.2f}%\n")
    f.write(f"  最大DD: {leverage_mdd_full*100:.2f}%\n")
    f.write(f"  シャープレシオ: {leverage_sharpe_full:.3f}\n\n")
    f.write("="*80 + "\n\n")
    ...
    if len(optimal_candidates) > 0:
        ...
        f.write(f"  レバレッジ戦略: {best['weight_leverage']*100:.0f}%\n\n")
        f.write(f"パフォーマンス:\n")
...
print(f"  レバレッジ戦略（単体）: {leverage_cagr_full*100:+6.2f}%  {leverage_mdd_full*100:7.2f}%  {leverage_sharpe_full:.3f}")
