"""
Target15_LB3（レバレッジなし）の月次リターンを抽出

目的: ベース戦略との組み合わせでMDDをさらに削減

パラメータ:
- Target volatility: 15%（レバレッジなし）
- Lookback: 3ヶ月
- Max leverage: 1.0（上限100%、レバレッジ禁止）
"""
...
print("="*80)
print("Target15_LB3（レバレッジなし）月次リターン抽出")
print("="*80)
...
rebalance_dates = [d for d in rebalance_dates if d <= pd.Timestamp('2026-01-31')]
...
# Target15_LB3を適用（レバレッジなし）
print("\n[3/3] Target15_LB3を適用中（レバレッジなし）...")

TARGET_VOL = 0.15  # 15%（レバレッジなし）
LOOKBACK_MONTHS = 3
MAX_POSITION = 1.0  # 上限100%（レバレッジ禁止）
...
for i in range(len(base_strategy_returns)):
    if i < LOOKBACK_MONTHS:
        ...
        if realized_vol > 0:
            ...
            position_size = np.clip(position_size, 0.20, MAX_POSITION)  # 20%～100%
        else:
            position_size = 1.0
...
# DataFrameに変換
df_target15_returns = pd.DataFrame({
    'year_month': month_labels_final,
    'return': adjusted_returns
})

# 月初に正規化
df_target15_returns['year_month'] = df_target15_returns['year_month'].dt.to_period('M').dt.to_timestamp()

print(f"Target15_LB3の月次リターン: {len(df_target15_returns)}ヶ月")

# パフォーマンス確認
cumulative_return = (1 + df_target15_returns['return']).cumprod().iloc[-1] - 1
years = len(df_target15_returns) / 12
cagr = (1 + cumulative_return) ** (1 / years) - 1

cumulative_series = (1 + df_target15_returns['return']).cumprod()
peak = cumulative_series.expanding(min_periods=1).max()
drawdown = (cumulative_series - peak) / peak
mdd = drawdown.min()

volatility = df_target15_returns['return'].std() * np.sqrt(12)
...
df_target15_returns.to_csv(output_dir / 'monthly_returns_target15_lb3.csv', index=False)

print(f"\n保存先: {output_dir / 'monthly_returns_target15_lb3.csv'}")
print("\n完了！")
