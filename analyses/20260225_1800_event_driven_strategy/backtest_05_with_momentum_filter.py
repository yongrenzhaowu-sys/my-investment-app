"""
月次リバランス戦略 + モメンタムフィルター（過去1ヶ月）

戦略: 小型株（時価総額Q1）× 高成長率（カスタム成長率Q4）
フィルター: ベンチマーク（全銘柄平均）の過去1ヶ月リターン > 0%の時のみエントリー
エントリー: 月初
エグジット: 翌月初
保有期間: 約1ヶ月

目的: 最大ドローダウンを削減し、リターンを維持
"""
...
print("="*80)
print("月次リバランス戦略 + モメンタムフィルター（過去1ヶ月）")
print("="*80)
...
# ================================================================================
# 4. バックテストの実行（モメンタムフィルター付き）
# ================================================================================

print("\n[4/5] バックテストを実行中...")
print("\n【重要】過去1ヶ月リターン > 0%の時のみエントリー、それ以外は現金保有")
...
for i in range(len(months) - 1):  # 最後の月はエグジットできないのでスキップ
    ...
    # ===== モメンタムフィルターの判定 =====
    # エントリー日の1ヶ月前（21営業日前）の価格を取得
    trading_days = list(df_price_pivot.index)
    if entry_date not in trading_days:
        print(f"    エントリー日が営業日リストにない、スキップ")
        continue

    entry_idx = trading_days.index(entry_date)
    if entry_idx < 21:
        print(f"    過去1ヶ月データなし、スキップ")
        continue

    one_month_ago_date = trading_days[entry_idx - 21]

    # 過去1ヶ月のリターンを計算
    current_benchmark = daily_benchmark.loc[entry_date]
    past_benchmark = daily_benchmark.loc[one_month_ago_date]
    momentum_return = (current_benchmark - past_benchmark) / past_benchmark

    momentum_filter_pass = momentum_return > 0

    if momentum_filter_pass:
        print(f"    [OK] モメンタムフィルターOK: 過去1ヶ月リターン {momentum_return*100:+.2f}% > 0%")
        print(f"    エントリー: {entry_date.date()}, エグジット: {exit_date.date()}")
    else:
        print(f"    [NG] モメンタムフィルターNG: 過去1ヶ月リターン {momentum_return*100:+.2f}% <= 0%")
        print(f"    -> 現金保有（リターン0%）")

    filter_status_list.append({
        'month': str(current_month),
        'filter_pass': momentum_filter_pass,
    ...
    month_records.append({
        'month': str(current_month),
        'entry_date': entry_date if momentum_filter_pass else None,
        'exit_date': exit_date if momentum_filter_pass else None,
...
# 保存
output_dir = PROJECT_ROOT / 'analyses/20260225_1800_event_driven_strategy/results_momentum_filter'
...
print("\n" + "="*80)
print("バックテスト完了（モメンタムフィルター付き）")
print("="*80)

print("\n【比較】")
print("  フィルターなし: 年率+21.21%, MDD -31.32%, シャープ 1.122")
print("  200日MAフィルター: 年率+5.61%, MDD -15.56%, シャープ 0.457")
print(f"  モメンタムフィルター: 年率{cagr_strategy*100:+.2f}%, MDD {mdd_strategy*100:.2f}%, シャープ {sharpe_strategy:.3f}")
print(f"\n  vs フィルターなし:")
print(f"    リターン差: {(cagr_strategy - 0.2121)*100:+.2f}%")
print(f"    MDD改善: {(mdd_strategy - (-0.3132))*100:+.2f}%")
print(f"    シャープ差: {sharpe_strategy - 1.122:+.3f}")
