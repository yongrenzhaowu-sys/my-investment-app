"""
年次リバランス（10月）バックテスト - tanh(slope/sigma)モメンタム戦略

【参考】https://overtheperiod.hatenablog.com/entry/2025/12/22/211113

【モメンタム指標の計算方法】
1. 過去90日分の対数終値を用いて線形回帰 → SLOPE（傾き）を抽出
2. 残差の標準偏差 → SIGMA（ボラティリティ）を計算
3. score = tanh(SLOPE / SIGMA) で正規化
4. scoreが高い銘柄を選択（期待値が高い = トレンドが強く、ボラティリティが低い）

【特徴】
- SLOPEをSIGMAで正規化 → シャープレシオ的な指標
- tanh変換で-1～+1に収束 → 極端な値を抑制
- 中期モメンタムの捕捉
"""
...
print("="*80)
print("年次リバランス（10月）バックテスト - tanh(slope/sigma)モメンタム戦略")
print("="*80)
...
# ================================================================================
# 3. モメンタム指標の計算（tanh(slope/sigma)）
# ================================================================================

print("\n[3/4] tanh(slope/sigma)モメンタム指標を計算...")

def calculate_tanh_sharpe_momentum(prices_df, date, lookback_days=90):
    """
    tanh(slope/sigma)モメンタム指標を計算
    ...
    for code in historical.columns:
        ...
        # slope/sigma比
        sharpe_like_ratio = slope / sigma

        # tanh変換（-1～+1に正規化）
        momentum_score = np.tanh(sharpe_like_ratio)
...
for i, rdate in enumerate(rebalance_dates):
    ...
    momentum = calculate_tanh_sharpe_momentum(df_price_pivot, rdate, lookback_days=90)
    momentum_by_date[rdate] = momentum

print("tanh(slope/sigma)モメンタム計算完了")

# ================================================================================
# 4. バックテスト実行（tanh(slope/sigma)モメンタム戦略）
# ================================================================================

print("\n[4/4] バックテストを実行（tanh(slope/sigma)モメンタム戦略）...")
...
for i in range(len(rebalance_dates) - 1):
    ...
    # 推奨閾値（tanh >= 0.95 ≈ slope/sigma >= 1.8）をクリアした銘柄数
    high_quality_count = (selected['momentum_score'] >= 0.95).sum()

    print(f"    選択: {len(selected)}銘柄, スコア範囲: {selected['momentum_score'].min():.4f}～{selected['momentum_score'].max():.4f}, 高品質: {high_quality_count}銘柄")
...
print("\n" + "="*80)
print("【tanh(slope/sigma)モメンタム戦略結果】")
print("="*80)
...
if len(results) == 0:
    ...
    df_results.to_csv(output_dir / 'momentum_tanh_sharpe_results.csv', index=False, encoding='utf-8-sig')

    with open(output_dir / 'momentum_tanh_sharpe_summary.txt', 'w', encoding='utf-8') as f:
        f.write("tanh(slope/sigma)モメンタム戦略\n")
        f.write("="*80 + "\n\n")
        f.write("【参考】https://overtheperiod.hatenablog.com/entry/2025/12/22/211113\n\n")
        f.write("【モメンタム計算方法】\n")
        f.write("1. 過去90日分の対数終値で線形回帰 → SLOPE\n")
        f.write("2. 残差の標準偏差 → SIGMA\n")
        f.write("3. score = tanh(SLOPE / SIGMA)\n")
        f.write("4. scoreが高い銘柄を選択\n\n")
        f.write("【特徴】\n")
        f.write("- SLOPEをSIGMAで正規化（シャープレシオ的）\n")
        f.write("- tanh変換で極端な値を抑制\n")
        f.write("- 推奨閾値: tanh >= 0.95 (slope/sigma >= 1.8)\n\n")
        f.write("="*80 + "\n\n")
    ...
    print(f"  モメンタム（6M）:             年率+24.94%")
    print(f"  モメンタム(tanh(slope/sigma)): 年率{annual_return*100:+.2f}%")

    if annual_return > 0.2561:
        print(f"  → ✅ tanh版がベースラインを上回りました！（+{annual_return*100 - 25.61:.2f}%pt）")
    elif annual_return > 0.2494:
        print(f"  → ✅ tanh版が6M版を上回りました（+{annual_return*100 - 24.94:.2f}%pt）")
    else:
        print(f"  → tanh版はベースラインを下回りました（{annual_return*100 - 25.61:.2f}%pt）")
    print("="*80)
