"""
年次リバランス（10月）バックテスト - R2補正モメンタム戦略

【モメンタム指標の計算方法】
1. 過去90日分の対数終値を用いて線形回帰
2. SLOPE（傾き）を抽出
3. 年率換算: (1 + SLOPE)^250 - 1
4. R2（決定係数）で補正: momentum_score = annualized_return × R2
...
print("="*80)
print("年次リバランス（10月）バックテスト - R2補正モメンタム戦略")
print("="*80)
...
def calculate_r2_adjusted_momentum(prices_df, date, lookback_days=90):
    """
    R2補正モメンタム指標を計算
    ...
    for code in historical.columns:
        ...
        # 年率換算（250営業日）
        # slope は1日あたりの対数リターン
        # 【修正】対数リターン → 単純リターンに変換してから年率化
        daily_simple_return = np.exp(slope) - 1  # exp(log_return) - 1 = simple_return
        annualized_return = (1 + daily_simple_return) ** 250 - 1

        # 【フィルタ2】異常な年率リターンを除外（-95%～+1000%の範囲内）
...
for i, rdate in enumerate(rebalance_dates):
    ...
    momentum = calculate_r2_adjusted_momentum(df_price_pivot, rdate, lookback_days=90)
    momentum_by_date[rdate] = momentum

print("R2補正モメンタム計算完了")

# ================================================================================
# 4. バックテスト実行（R2補正モメンタム戦略）
# ================================================================================

print("\n[4/4] バックテストを実行（R2補正モメンタム戦略）...")
...
for i in range(len(rebalance_dates) - 1):
    ...
    print(f"    選択銘柄数: {len(selected)}, モメンタム範囲: {selected['momentum_score'].min():.4f} ~ {selected['momentum_score'].max():.4f}")
...
if len(results) == 0:
    ...
    df_results.to_csv(output_dir / 'momentum_r2_adjusted_results.csv', index=False, encoding='utf-8-sig')

    with open(output_dir / 'momentum_r2_adjusted_summary.txt', 'w', encoding='utf-8') as f:
        f.write("R2補正モメンタム戦略\n")
        f.write("="*80 + "\n\n")
        f.write("【モメンタム計算方法】\n")
        f.write("1. 過去90日分の対数終値で線形回帰\n")
        f.write("2. SLOPE（傾き）を抽出\n")
        f.write("3. 年率換算: (1 + SLOPE)^250 - 1\n")
        f.write("4. R2で補正: momentum_score = annualized_return × R2\n\n")
        f.write("【特徴】\n")
        f.write("- トレンドが強い（高R2）銘柄を優先\n")
        f.write("- ボラティリティが高い銘柄を減点\n")
        f.write("- より安定したモメンタムを捕捉\n\n")
        f.write("="*80 + "\n\n")
    ...
    print(f"  モメンタム（R2補正）:      年率{annual_return*100:+.2f}%")

    if annual_return > 0.2561:
        print(f"  → R2補正版がベースラインを上回りました！（+{annual_return*100 - 25.61:.2f}%pt）")
    elif annual_return > 0.2494:
        print(f"  → R2補正版が6M版を上回りました（+{annual_return*100 - 24.94:.2f}%pt）")
    else:
        print(f"  → R2補正版はベースラインを下回りました（{annual_return*100 - 25.61:.2f}%pt）")
    print("="*80)
