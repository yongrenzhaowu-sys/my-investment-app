"""
年次リバランス（10月）バックテスト - 理論株価×モメンタム複合戦略

【戦略コンセプト】
1. カスタム成長率を用いて来期の理論株価を計算
2. 現株価との乖離が大きい銘柄（割安）を抽出
3. その中からモメンタム指標が高い銘柄を購入
...
if len(results) == 0:
    ...
    df_results.to_csv(output_dir / 'theoretical_price_momentum_results.csv', index=False, encoding='utf-8-sig')
