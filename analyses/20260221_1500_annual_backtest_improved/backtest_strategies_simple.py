"""
年次リバランス（10月）バックテスト - 改善版

【Legacy版からの改善点】
1. ルックアヘッド対策: リバランス日以降の価格のみ使用
2. 現金管理: 現金残高を明示的に追跡
3. 税金計算: 暦年ベースで損益通算
4. 投資比率の正確な計算
"""
...
print("="*80)
print("年次リバランス（10月）バックテスト - 改善版（現実的な実装）")
print("="*80)
...
# ================================================================================
# 4. バックテスト実行（改善版：現実的な実装）
# ================================================================================

print("\n[4/4] バックテストを実行（改善版：現実的な実装）...")
...
for i in range(len(rebalance_dates) - 1):
    ...
    sell_value = 0
    if len(portfolio) > 0:
        # 【改善1】売却価格はリバランス日以降の価格のみ使用（ルックアヘッド対策）
...
if len(results) == 0:
    ...
    df_results.to_csv(output_dir / 'annual_results_improved.csv', index=False, encoding='utf-8-sig')

    with open(output_dir / 'summary_improved.txt', 'w', encoding='utf-8') as f:
        f.write("年次リバランス（10月）バックテスト - 改善版\n")
        f.write("="*80 + "\n\n")
        f.write("【改善点】\n")
        f.write("1. ルックアヘッド対策: リバランス日以降の価格のみ使用\n")
        f.write("2. 現金管理: 現金残高を明示的に追跡\n")
        f.write("3. 税金計算: 暦年ベースで損益通算\n")
        f.write("4. 投資比率の正確な計算\n\n")
        f.write("="*80 + "\n\n")
    ...
    print("\n" + "="*80)
    print("Legacy版（年率+28.52%）および再現版（+24.27%）との比較:")
    print(f"  Legacy版: 年率+28.52%")
    print(f"  再現版:   年率+24.27%")
    print(f"  改善版:   年率{annual_return*100:+.2f}%")
    print(f"  vs Legacy: {annual_return*100 - 28.52:+.2f}%pt")
    print(f"  vs 再現版: {annual_return*100 - 24.27:+.2f}%pt")
    print("="*80)
