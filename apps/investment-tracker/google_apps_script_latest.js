...
function doPost(e) {
  try {
    ...
    } else if (action === "save_additional_investments") {
      // 追加投資の保存（新規）
      return saveAdditionalInvestments(params.data);
    } else if (action === "save_option_trades") {
      // オプション取引の保存（新規）
      return saveOptionTrades(params.data);
    } else {
 ...
 * 追加投資を保存（新規機能）
 */
function saveAdditionalInvestments(data) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  // 追加資金シートを取得（なければ作成）
  let sheet = ss.getSheetByName("追加資金");
  if (!sheet) {
    sheet = ss.insertSheet("追加資金");
  }
 ...
 * オプション取引を保存（新規機能）
 */
function saveOptionTrades(data) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  // オプション取引シートを取得（なければ作成）
  let sheet = ss.getSheetByName("オプション取引");
  if (!sheet) {
    sheet = ss.insertSheet("オプション取引");
  }
  ...
  // ヘッダー行
  const headers = ["id", "date", "description", "profit", "created_at"];
  sheet.appendRow(headers);
